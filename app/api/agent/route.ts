...
// DistributedRateLimiter enforces both a per-second burst cap and a per-hour
// volume cap per IP. When KV env vars are set, state is shared across all
// Lambda instances; otherwise it falls back to per-instance in-memory buckets.
...
import { DistributedRateLimiter, kvFromEnv } from "@/lib/agent/distributed-rate-limiter";
...
//
// Build once per Lambda warm invocation. When KV is configured, this uses
// shared state across instances — defeating the per-instance escape that
// would otherwise multiply the limit by the fan-out factor.
const RATE_LIMIT = new DistributedRateLimiter({
  perSecond: 5,
  perMinute: 100,
  kv: kvFromEnv(),
});
...
export async function POST(req: NextRequest) {
  ...
  const rl = await RATE_LIMIT.consume(ip);
  if (!rl.allowed) {
    return new Response(
      JSON.stringify({
        error: RATE_LIMIT.isDistributed()
          ? "Rate limit reached. Public demo capped at 5 req/sec, 100 req/hr per IP across all server instances."
          : "Rate limit reached (single-instance fallback). Public demo capped at 5 req/sec, 100 req/hr per IP. Note: serverless deployment without KV uses per-instance buckets — the cap is approximate.",
